...
public class FireworkCreator : MonoBehaviour
{
    ...
    //오브 교체
    void ChangeOrb(GameObject changeOrb)
    {
        usingOrb = changeOrb;
        Orb currOrb = usingOrb.GetComponent<Orb>();
        orbNumbers = currOrb.orbNumbers;
        energyCost = currOrb.energyNeeded;
    }
    ...
    List<Vector2> GetEmptySpace()
    {
        ...
        for (int i = 0; i < maxAttempts; i++)
        {
            ...
            if (coll == null)
            {
                ...
                if (points.Count >= orbNumbers) break;
            }
        }

        //갯수 부족인 경우 추가해서 최대 수를 맞춰준다. 
        if(points.Count < orbNumbers)
        {
            int dificientNumbers = orbNumbers - points.Count;
    ...
    IEnumerator LaunchFireWork()
    {
        ...
        for(int i = 0; i < orbNumbers; i++)
        {
            // 폭죽 프리팹을 랜덤한 위치에 생성합니다.
            GameObject firework = PoolManager.instance.GetItem(fireworkPrefab);
    ...
    IEnumerator MoveAndExplode(GameObject firework, Vector2 targetPos)
    {
        ...
        // 오브 생성.
        GameObject newOrb = PoolManager.instance.GetItem(usingOrb);
